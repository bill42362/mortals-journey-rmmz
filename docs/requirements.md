...
8. 檔案結構幫我這樣安排，最終目的刪除 requirements.md:
    ...
    b. docs/TECH.md: 技術文件，包含但不限於技術選型，專案架構，專案元件規劃，技術債追蹤等等。
    c. docs/ROADMAP.md: 遊戲開發的工程階段以及追蹤列表。
    d. docs/story/: 遊戲各章節的細部資料，包含但不限於：劇情大綱，地圖規劃，地圖事件，對話文本等等。
    e. README.md: 給技術人員初次接觸專案時快速理解專案內容用，包含簡短的專案描述，各元件架構，如何在本地一鍵運行專案等等。但因為目前還沒有進入開發階段，所以應該幾乎沒有內容。
    f. CLAUDE.md: 給 AI 閱讀用的 context，包含需要紀錄但不適合寫入以上其他檔案的內容，相關決策點，待決策項目等等。
    g. game/: 先暫時留空，目前還在文件製作階段。
