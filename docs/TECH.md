...
Demo 啟用 **9 個 VisuStella 模組**（原 10；`SkillLearnSystem` 已於 2026-07-06 移除，功法改原生書簡參悟，見 §9.3）。**載入順序 = tier 數字升冪**（檔名前綴 `VisuMZ_<tier>_` 即 tier；低 tier 先載、高 tier 後載），下表已按外掛管理器（Plugin Manager）由上至下的正確順序排列：
...
| 9 | `VisuMZ_2_ItemCraftingSys` | 2 | 配方合成 | **煉丹**（製符同框架） | CoreEngine；**硬性需 ItemsEquipsCore（須在其上）** |
...
4. **功法習得改原生機制**（2026-07-06）：demo 以**原生「習得技能」道具效果**（書簡/玉簡參悟）＋事件師授實作功法，**已移除** `VisuMZ_2_SkillLearnSystem`；靈石花在買典籍。日後若需瀏覽式參悟選單再加回。
...
> 來源：[Core Engine](https://www.yanfly.moe/wiki/Core_Engine_VisuStella_MZ)、[Item Crafting System](https://www.yanfly.moe/wiki/Item_Crafting_System_VisuStella_MZ)（Yanfly.moe Wiki）。
>
> ✅ **實機載入驗證 (B) 通過（2026-06-30）**：10 個外掛照序啟用（含當時的 SkillLearnSystem；2026-07-06 移除後為 9，待下次 playtest 復驗）、playtest 無紅字報錯、smoke test 通過。安裝 SOP 見 [PLUGINS.md](./PLUGINS.md)。

### 1.2 需自製的外掛

| 外掛 | 職責 | 備註 |
| --- | --- | --- |
| `MJ_GreenBottle.js` | ✅ 已實作（2026-07-05）：小綠瓶承月光滋生靈液，以靈液澆灌加速生長 | 靈液＝變數3；供 HerbFarming 消耗。核心樞紐。詳見 [PLUGINS §2.2](./PLUGINS.md) |
| `MJ_HerbFarming.js` | ✅ 已實作（2026-07-05）：多地塊狀態機（播種→生長→可採），自然生長靠步數、澆灌靈液催長 | 依賴 MJ_GreenBottle；地塊存 `$gameSystem` |
| `MJ_Realm.js` | ✅ 已實作（2026-07-05）：境界鏡像變數(變數2)、瓶頸(等級上限)、突破(Change Class)封裝 | `Classes.json` 需 練氣(9)/築基(10)；見 §3 |
| `MJ_LootAndPK.js` | ✅ 已實作（2026-07-06）：屍體掉寶(機率/具名表)＋伏擊判定(先制/被偷襲) | 功法**不**在此習得(走書簡道具)；輕量，事件為主 |
...
> **遇敵相容性慣例（2026-07-06 釐清）**：可見踩觸遇敵（事件戰鬥，先攻走 `MJ_LootAndPK` 伏擊判定）與原生**隨機遇敵**（先攻走原生 `onEncounter`）為兩套獨立系統、**互不衝突**，可同圖並存。原則：**主線以可見遇敵為主**（承載「苟/避戰」）；**特定險地**（如血色試煉毒瘴、密林）可用 RMMZ 的 **Region 限定隨機遇敵**（Encounters 綁 region）承載「資源即生命/凶險莫測」——安全路徑看得見敵人可繞、踏進瘴氣區才隨機遭遇。程式面無需再改外掛，屬地圖/資料設定。
...
**核心：職業=境界、等級=層、等級上限=撞瓶頸、突破=事件、功法=書簡參悟習得。**
...
- **功法 = 原生「習得技能」道具效果（書簡/玉簡參悟）＋事件師授**：跨境界保留。acquisition＝師授（如長春功）或取得書簡後使用參悟（書簡**不消耗**）；靈石花在**買典籍/玉簡**，非「習得費」。（原規劃用 VisuStella SkillLearnSystem，2026-07-06 改原生、外掛已移除，貼原著。）
- **境界鏡像變數**：`MJ_Realm.js` 維護一個變數同步當前境界，供事件條件判斷（比查 class 省事）。

> ✅ **已實作（2026-07-05）**：`MJ_Realm.js` 一次設計三境界。`Classes.json` 已新增 **練氣(classId 9)／築基(classId 10)**（clone 凡人）；境界鏡像＝**變數 2**（1凡/2練/3築，新遊戲/突破自動同步）；覆寫 `Game_Actor.maxLevel` 實作撞瓶頸，層數上限暫定 **凡人 3／練氣 13／築基 10**（改 `MJ_Realm.js` 的 `REALMS` 常數）。突破＝插件指令 `境界突破`（`changeClass(下一境界, keepExp=false)` 落新境界一層）；條件檢查與文字演出由呼叫端共通事件負責。三職業成長曲線數值仍為 placeholder（待 [ROADMAP](./ROADMAP.md)「境界職業參數草案」＋ M4）。
...
| # | 系統 | 實作路徑 | 主要依賴 |
| --- | --- | --- | --- |
| 1 | 神秘小綠瓶 | 自製外掛：承月光滋生靈液（變數計時），靈液澆灌驅動種植加速 | `MJ_GreenBottle.js` |
| 2 | 靈草種植/催化 | 自製外掛：地塊狀態機（播種→生長→可採），受小綠瓶加速 | `MJ_HerbFarming.js` |
| 3 | 技能/功法 | 原生技能 + **原生「習得技能」道具效果（書簡參悟）** / 事件師授 | `SkillsStatesCore` |
...
> 註：典籍參悟＝原生「習得技能」道具效果（書簡/玉簡）；製符視為煉丹（Item Crafting）框架的延伸，demo 視進度納入。
...
> - 七玄門：正統宗門
> - 黃楓谷百藥園：藥香靈秀
> - 血色試煉之地：詭譎驚悚（可用 **Cthulhu Mythos 包 b** 營造試煉禁地恐怖感）
>
...
| 種植系統自製 vs 現成外掛 | 🟢 已自製並實作（2026-07-05） | `MJ_GreenBottle`＋`MJ_HerbFarming`，與小綠瓶耦合（見 §1.2 / PLUGINS §2.2） |
| 核心系統外掛（境界/小綠瓶/種植/殺人奪寶） | 🟢 已實作（2026-07-05/06） | `MJ_Realm`/`MJ_GreenBottle`/`MJ_HerbFarming`/`MJ_LootAndPK`；數值 placeholder 待 M4 |
| 第一幕資料層（道具/功法/煉丹配方/習得成本） | 🟢 已配置（2026-07-06） | 見 [PLUGINS §2.3](./PLUGINS.md)；第二幕靈石成本待配 |
...
| **低階靈石** | **可堆疊道具（key item）** | 修仙界**硬通貨**：功法習得、煉丹素材交易、突破、黑市、殺人奪寶掉落 | **第一幕末少量初見**（墨大夫賞賜/遺澤）；**第二幕黃楓谷起**為硬通貨 |

### 9.2 為何靈石用「道具」而非第二個原生貨幣

1. **承載「資源即生命」**：靈石是看得見、會心疼、每次花用都是明確抉擇的**囤積資源**，而非後台自動扣的數字。
2. **與外掛生態天生契合**：買功法典籍/玉簡（兌換窗以靈石購入）、煉丹（ItemCraftingSys 消耗素材）、殺人奪寶（`MJ_LootAndPK` 掉靈石/丹藥/書簡）——整條 loot→craft→買書參悟 迴圈都吃道具。
...
- **功法習得**：靈石花在**購買功法典籍/玉簡**（坊市/藏經閣兌換窗）；取得後以原生「習得技能」道具效果**參悟習得**，**書簡不消耗**（貼原著；非「習得費」）。
- **煉丹/製符**：ItemCraftingSys 配方以靈石為素材/成本之一。
- **一般兌換（買賣丹藥/素材）**：**共通事件兌換窗**（選項＋靈石道具數量條件判斷），或日後一支小 `MJ_` 外掛統一處理。
- **殺人奪寶掉落**：`MJ_LootAndPK` 屍體掉靈石/丹藥道具。
- **回售與購入（閉環）**：兌換窗**雙向**——賣靈草/靈丹得靈石、買稀有靈藥種子與飛劍法器（金蚨子母刃）；與「催生→煉丹」構成生財閉環（見下 §9.4）。

**靈石生財閉環（黃楓谷經濟主軸，✅ 定案 2026-07-04）**：**小綠瓶催生靈藥 → 坊市賣得靈石 → 靈石換功法（青元劍訣/長春功下半）／丹方（黃龍丹/金髓丸/築基丹）／稀有靈藥種子／飛劍法器（金蚨子母刃） → 得丹方開煉丹、產真靈丹 → 回坊市賣得更多靈石**。小綠瓶是閉環引擎（加速靈藥＝加速產出）；靈石需求量大，凸顯「小瓶是命脈」。稀有種子部分坊市靈石購、**某一味必需種子只能打百藥園山獸得**。（劇情落點見 [story/02 §7.3–7.6](story/02-第二幕-黃楓谷.md)。）

### 9.4 Demo 範圍與分章呈現

- **品階**：demo **只用低階靈石一階**（中/高階延後），降複雜度。
- **序章（青牛鎮·凡人）**：只有銅板，**靈石不出現**（與「序章不出現修為/小綠瓶」一致，製造期待）。
- **第一幕（七玄門·凡人→練氣）**：外門月例以米糧/銅板呈現凡俗底層；末段**少量初見**低階靈石（墨大夫賞賜/遺澤），但「靈石＝硬通貨、靈石換功法典籍」的**雙層貨幣正式切換延到第二幕黃楓谷**（七玄門維持凡俗經濟；真靈丹煉製、築基丹丹方亦在黃楓谷首登場）。
- **第二～壓軸（黃楓谷·練氣）**：**靈石正式成為硬通貨**（靈石換功法典籍、真靈丹煉製、築基丹丹方均自此登場）；採藥/煉丹/殺人奪寶全圍繞靈石與丹藥稀缺。並以「催生靈藥→賣靈石→換功法/丹方/種子/法器→煉丹→回售」形成**生財閉環**（小綠瓶為引擎，見 §9.3）。
...
| 技能(skill) | **功法** | 習得走書簡參悟（原生效果）/事件師授 |

**狀態**：`System.json terms`（基本/屬性/指令/戰鬥訊息）✅ 已全數中文化；`skillTypes[2]` 已正名「**功法**」（韓立功法用此 skill type，見 [PLUGINS §2.3](./PLUGINS.md)）。**待補**：VisuStella 各外掛的參數字串（OptionsCore 選項名、煉丹/習得介面字串等），屬「外掛參數層」，漸進補譯（煉丹/習得選單名已設「煉製」「參悟」但入口關閉、事件驅動）。
