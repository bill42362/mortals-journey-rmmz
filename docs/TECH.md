...
| --- | --- | :---: | --- | --- | --- |
| 1 | `VisuMZ_0_CoreEngine` | 0 | 基礎依賴、解析度設定 | 全部、§1.4 解析度 | — |
...
> ⚠️ 載入位置：**掛鉤 VisuStella** 的 `MJ_*.js`（GreenBottle/HerbFarming/Realm/LootAndPK）須排在 §1.1 全部 VisuStella 模組**之下**（清單最末）。**不依賴 VisuStella 的獨立覆寫**（如 `MJ_FillWindow`，僅改 `Graphics`）位置不拘。
...
**⚠️ 已知限制（待人工驗證 B）**：`\tl{}` 在「會解析文字代碼處」最穩（對話訊息）。**資料庫名稱**（道具/技能/職業名，存 `data/*.json`）與**外掛參數文字**不一定吃 `\tl{}`——要做到「遊戲內 100% 切語言」這些需逐一驗證對策。對 demo（中文為主）無礙；真正多語系上架前須確認 DB/參數類文本的本地化途徑。
...
| 決策 | 定案 | 為何現在鎖 | 實作 |
| --- | --- | --- | --- |
| **螢幕解析度** | **1280×720（16:9）** | 地圖照解析度繪製，晚改＝重畫所有地圖 | `CoreEngine` 參數，動工前設定 |
| **遭遇戰形式** | **可見踩觸遇敵**（地圖上看得見敵人，可繞過/埋伏/被偷襲） | 決定每張地圖敵人事件擺法；直接承載「苟/避戰/先制/被偷襲」核心精神 | 既有 `EventsMoveCore` + `MJ_LootAndPK`（視野/追逐/伏擊），**無需新外掛** |
| **中文字型內嵌** | **打包開源 CJK 字型**（建議思源黑體 / Noto Sans CJK TC，SIL OFL 授權，可商用可散布） | 網頁部署預設字型不保證涵蓋全中文 → 某些瀏覽器出「豆腐方塊」；與 i18n 逐語言字型綁一起做 | 字型檔入 `game/fonts/`、`gamefont.css`/MessageCore 設定 |
